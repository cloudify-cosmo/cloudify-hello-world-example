...
cfy_info "Starting HTTP server from ${PYTHON_FILE_SERVER_ROOT}"

echo "Changing directory to ${PYTHON_FILE_SERVER_ROOT}"
cd ${PYTHON_FILE_SERVER_ROOT}
cfy_info "Starting SimpleHTTPServer"
nohup python -m SimpleHTTPServer ${port} > /dev/null 2>&1 &
echo $! > ${PID_FILE}

cfy_info "Waiting for server to launch"

STARTED=false
for i in $(seq 1 15)
do
	if wget http://localhost:${port} 2>/dev/null ; then
		cfy_info "Server is up."
		STARTED=true
    	break
	else
		cfy_info "Server not up. waiting 1 second."
		sleep 1
	fi	
done
